...
ttObjects.room.removeDj_ttTools=ttObjects.room.removeDj;ttObjects.room.removeDj=function(uid){ttTools.autoDJ.execute();this.removeDj_ttTools(uid);}},checkVersion:function(){if(parseInt($.cookie('ttTools_release'))!==ttTools.release.getTime()){$.cookie('ttTools_release',ttTools.release.getTime());this.views.info.render();}},timestamp:function(millis){millis=util.now()-millis;if(millis<ttTools.constants.time.minutes)
...
ttTools.release=new Date(1342670276000);$.when(ttTools.roomLoaded()).then($.proxy(ttTools.init,ttTools));