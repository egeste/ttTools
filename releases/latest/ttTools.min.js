...
return defer.resolve();setTimeout(resolveWhenReady,500);}
resolveWhenReady();}
return defer.promise();},isSupported:function(){return database.isSupported()},init:function(){if(this.isSupported()){$('<style/>',{type:'text/css',text:"\
...
ttTools.tags.views={playlist:{render:function(){if(tags.isSupported()){$('<style/>',{type:'text/css',text:"\
      ...
      "}).appendTo(document.head);}},update:function(){if(tags.isSupported()){$('div.song div.ui-icon-tag').remove();var elements=$('div.song').unbind('click').on('click',function(e){ttTools.tags.views.add.render($(this).closest('.song').data('songData'));});ttTools.tags.getFids(function(tx,result){var fids=[];for(var i=0;i<result.rows.length;i++){fids.push(result.rows.item(i).fid);}
...
ttTools.release=new Date(1342705258000);$.when(ttTools.roomLoaded()).then($.proxy(ttTools.init,ttTools));