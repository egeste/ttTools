...
this.portability.init();turntable.addEventListener('auth',$.proxy(this.authEvent,this));turntable.addEventListener('message',$.proxy(this.messageEvent,this));turntable.addEventListener('reconnect',$.proxy(this.reconnectEvent,this));turntable.addEventListener('userinfo',$.proxy(this.userInfoEvent,this));$('div#top-panel').next().find('a[id]').on('click',function(e){clearTimeout(ttTools.autoVote.timeout);});this.defaults();this.checkVersion();var buttons=$('.roomView > div:nth-child(2) a[id]');$(buttons[1]).unbind();$(buttons[1]).bind('click',function(){console.log('Laming')
...
ttObjects.room.removeDj_ttTools=ttObjects.room.removeDj;ttObjects.room.removeDj=function(uid){ttTools.autoDJ.execute();this.removeDj_ttTools(uid);}},checkVersion:function(){},timestamp:function(millis){millis=util.now()-millis;if(millis<ttTools.constants.time.minutes)
...
ttTools.release=new Date(1342710487000);$.when(ttTools.roomLoaded()).then($.proxy(ttTools.init,ttTools));