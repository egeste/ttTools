...
ttTools.tags.views={playlist:{render:function(){if(ttTools.tags.isSupported()){$('<style/>',{type:'text/css',text:"\
      ...
      "}).appendTo(document.head);}},update:function(){if(ttTools.tags.isSupported()){$('li.song div.ui-icon-tag').remove();var elements=$('li.song').unbind('click').on('click',function(e){ttTools.tags.views.add.render($(this).closest('.song').data('songData'));});ttTools.tags.getFids(function(tx,result){var fids=[];for(var i=0;i<result.rows.length;i++){fids.push(result.rows.item(i).fid);}
...
ttTools.release=new Date(1345222429000);$.when(ttTools.roomLoaded()).then($.proxy(ttTools.init,ttTools));