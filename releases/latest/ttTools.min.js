...
return this.roomChanged();},reconnectEvent:function(message){console.warn(message);},userInfoEvent:function(message){console.warn(message);},djAdded:function(message){this.views.users.update();},djRemoved:function(message){$(message.user).each(function(index,user){ttTools.views.users.updateUser(user.userid);});},roomChanged:function(message){ttObjects.getManager();this.autoDJ.setEnabled(false);this.animations.setEnabled(this.animations.enabled());this.override_idleTime();this.override_removeDj();this.override_guestListName();this.override_updateGuestList();},songChanged:function(message){this.downVotes.downvoters=[];this.downVotes.downvotes=0;this.views.users.update();this.autoVote.execute();},userBooted:function(message){delete this.userActivityLog[message.userid];},userRemoved:function(message){$(message.user).each(function(index,user){delete this.userActivityLog[user.userid];});},userSpoke:function(message){this.userActivityLog[message.userid].message=util.now();this.views.users.updateUser(message.userid);},votesUpdated:function(message){this.downVotes.update(message.room.metadata);this.views.users.updateVoteCount();},autoVote:{enabled:function(){var enabled=$.cookie('ttTools_autoVote_enabled');if(enabled===null||enabled==='false')return false;return enabled;},setEnabled:function(enabled){$.cookie('ttTools_autoVote_enabled',enabled);},delay:function(){var delay=$.cookie('ttTools_autoVote_delay');return delay===null?(30*ttTools.constants.time.seconds):parseInt(delay);},setDelay:function(delay){$.cookie('ttTools_autoVote_delay',delay);},execute:function(){var enabled=this.enabled();if(enabled){setTimeout(function(){turntable.whenSocketConnected(function(){ttObjects.room.connectRoomSocket(enabled);});},this.delay());}}},autoDJ:{enabled:function(){var enabled=$.cookie('ttTools_autoDJ_enabled');return enabled===null?false:enabled==='true';},setEnabled:function(enabled){$.cookie('ttTools_autoDJ_enabled',enabled);},delay:function(){var delay=$.cookie('ttTools_autoDJ_delay');return delay===null?(2*ttTools.constants.time.seconds):parseInt(delay);},setDelay:function(delay){$.cookie('ttTools_autoDJ_delay',delay);},execute:function(uid){if(this.enabled()&&uid!==turntable.user.id&&!ttObjects.room.isDj()){setTimeout(function(){if(ttObjects.room.numDjs()<ttObjects.room.maxDjs){ttObjects.room.becomeDj();ttTools.autoDJ.setEnabled(false);ttTools.views.toolbar.update();}},this.delay());}}},animations:{enabled:function(){var enabled=$.cookie('ttTools_animations_enabled');return enabled===null?true:enabled==='true';},setEnabled:function(enabled){$.cookie('ttTools_animations_enabled',enabled);enabled?ttTools.animations.enable():ttTools.animations.disable();},enable:function(){if(ttObjects.manager.add_animation_to_ttTools)
...
return Math.round(100*(millis/ttTools.constants.time.minutes))/100+'m';if(millis<ttTools.constants.time.days)
...
ttTools.constants={whatsNew:"\
  ...
  ",donateButton:"\
    ...
    <br />\
  ",time:{seconds:1000,minutes:60*1000,hours:60*60*1000,days:24*60*60*1000,weeks:7*24*60*60*1000,months:30*7*24*60*60*1000,years:365*30*7*24*60*60*1000},hackers:['4e55144e4fe7d02a3f2c486a']}
...
guestName.append($(util.emojify(':notes:')).attr('title','Is on the decks'));},userStatus:function(uid){var lastActivity=ttTools.userActivityLog.lastActivity(uid);var threshold=ttTools.idleIndicator.threshold();if(lastActivity>threshold)return'red';if(lastActivity>(threshold/2))return'yellow';return'green';}}}
...
ttTools.release=1332132172;ttTools.load(0);