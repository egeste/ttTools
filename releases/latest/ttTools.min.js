...
div.modal ul li {\
  ...
  text-align:left;\
      ...
      "}).appendTo($('div.modal'));$('div.modal div:first').html('').append(ttTools.constants.whatsNew()).append(ttTools.constants.submitIssue()).append(ttTools.constants.donateButton());}},toolbar:{render:function(){turntable.playlist.setPlaylistHeight($('div.chat-container').css('top').replace('px',''));$('#songs').addClass('top65')
...
$('ul.songs li.song:hidden').each(function(index,element){var element=$(element);var fid=element.data('songData').fileId;if($.inArray(fid,fids)>-1)
...
ttTools.tags.views={playlist:{render:function(){if(ttTools.tags.isSupported()){$('<style/>',{type:'text/css',text:"\
      ...
      "}).appendTo(document.head);}},update:function(){if(ttTools.tags.isSupported()){$('li.song div.ui-icon-tag').remove();var elements=$('li.song .title').unbind('click').on('click',function(e){ttTools.tags.views.add.render($(this).closest('.song').data('songData'));});ttTools.tags.getFids(function(tx,result){var fids=[];for(var i=0;i<result.rows.length;i++){fids.push(result.rows.item(i).fid);}
...
ttTools.release=new Date(1345235268000);$.when(ttTools.roomLoaded()).then($.proxy(ttTools.init,ttTools));