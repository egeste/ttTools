...
return false;}};ttTools={roomLoaded:function(){var defer=$.Deferred();var resolveWhenReady=function(){if(turntable&&ttObjects.getManager()&&ttObjects.getApi())
return defer.resolve();setTimeout(resolveWhenReady,500);}
resolveWhenReady();return defer.promise();},init:function(){$('<link/>',{type:'text/css',rel:'stylesheet',href:'http://ajax.googleapis.com/ajax/libs/jqueryui/1.8.1/themes/sunny/jquery-ui.css'}).appendTo(document.head);this.views.menu.render();this.views.users.render();this.views.toolbar.render();this.views.playlist.render();$.when(ttTools.tags.playlistLoaded()).then($.proxy(ttTools.tags.init,ttTools.tags))
this.portability.init();turntable.addEventListener('auth',$.proxy(this.authEvent,this));turntable.addEventListener('message',$.proxy(this.messageEvent,this));turntable.addEventListener('reconnect',$.proxy(this.reconnectEvent,this));turntable.addEventListener('userinfo',$.proxy(this.userInfoEvent,this));$('div#top-panel').next().find('a[id]').on('click',function(e){clearTimeout(ttTools.autoVote.timeout);});this.defaults();this.checkVersion();this.enhanceLameButton();},defaults:function(){clearTimeout(this.autoDJ.timeout);clearTimeout(this.autoVote.timeout);this.userActivityLog.init();this.autoDJ.setEnabled(false);this.autoVote.setEnabled(this.autoVote.enabled());this.autoVote.execute();this.animations.setEnabled(this.animations.enabled());this.downVotes.downvotes=0;this.downVotes.downvoters=[];this.override_idleTime();this.override_removeDj();this.override_guestListName();this.override_updateGuestList();this.override_setPlaylistHeight();this.views.playlist.update();this.views.users.modifyContainer();ttObjects.room.updateGuestList();$.when(ttTools.tags.playlistLoaded()).then($.proxy(ttTools.tags.defaults,ttTools.tags));},authEvent:function(message){console.warn(message);},messageEvent:function(message){if(typeof message.msgid!=='undefined'){if(typeof message.users!=='undefined'&&typeof message.room==='object'&&typeof message.room.metadata==='object'&&typeof message.room.metadata.songlog!=='undefined'){return $.when(this.roomLoaded()).then($.proxy(this.roomChanged,this));}
return;}
switch(message.command){case'speak':return this.userSpoke(message);case'update_votes':return this.votesUpdated(message);case'deregistered':return this.userRemoved(message);case'newsong':return this.songChanged(message);case'add_dj':return this.djAdded(message);case'booted_user':return this.userBooted(message);case'rem_dj':return this.djRemoved(message);case'playlist_complete':return this.playlistComplete(message);case'search_complete':return this.searchComplete(message);case'registered':case'snagged':case'update_user':return;default:return console.warn(message);}},reconnectEvent:function(message){console.warn(message);},userInfoEvent:function(message){console.warn(message);},djAdded:function(message){this.views.users.update();},djRemoved:function(message){$(message.user).each(function(index,user){ttTools.views.users.updateUser(user.userid);});},playlistComplete:function(message){this.views.playlist.update();this.tags.views.playlist.update();},roomChanged:function(){this.defaults();},searchComplete:function(message){this.tags.views.playlist.update();},songChanged:function(message){this.downVotes.downvoters=[];this.downVotes.downvotes=0;this.views.users.update();this.autoVote.execute();this.autoSongDrop.execute(message);},userBooted:function(message){delete this.userActivityLog[message.userid];},userRemoved:function(message){$(message.user).each(function(index,user){delete this.userActivityLog[user.userid];});},userSpoke:function(message){this.userActivityLog[message.userid].message=util.now();this.views.users.updateUser(message.userid);},votesUpdated:function(message){this.downVotes.update(message.room.metadata);this.views.users.updateVoteCount();},autoVote:{timeout:null,enabled:function(){var enabled=$.cookie('ttTools_autoVote_enabled');if(enabled===null||enabled==='false')return false;return'up';},setEnabled:function(enabled){$.cookie('ttTools_autoVote_enabled',enabled);},delay:function(){var delay=$.cookie('ttTools_autoVote_delay');return delay===null?(30*ttTools.constants.time.seconds):parseInt(delay);},setDelay:function(delay){$.cookie('ttTools_autoVote_delay',delay);},execute:function(){clearTimeout(this.timeout);var enabled=this.enabled();if(enabled){this.timeout=setTimeout(function(){if(ttObjects.room.currentSong){ttObjects.api({api:'room.vote',roomid:ttObjects.room.roomId,val:enabled,vh:$.sha1(ttObjects.room.roomId+enabled+ttObjects.room.currentSong._id),th:$.sha1(Math.random()+""),ph:$.sha1(Math.random()+"")});}},this.delay());}}},autoDJ:{timeout:null,enabled:function(){var enabled=$.cookie('ttTools_autoDJ_enabled');return enabled===null?false:enabled==='true';},setEnabled:function(enabled){$.cookie('ttTools_autoDJ_enabled',enabled);},delay:function(){var delay=$.cookie('ttTools_autoDJ_delay');return delay===null?(2*ttTools.constants.time.seconds):parseInt(delay);},setDelay:function(delay){$.cookie('ttTools_autoDJ_delay',delay);},execute:function(uid){clearTimeout(this.timeout);if(this.enabled()&&uid!==turntable.user.id&&!ttObjects.room.isDj()){this.timeout=setTimeout(function(){if(ttObjects.room.numDjs()<ttObjects.room.maxDjs){ttObjects.api({api:"room.add_dj",roomid:ttObjects.room.roomId},function(response){if(!response.success&&!ttObjects.room.isDj())return;ttTools.autoDJ.setEnabled(false);ttTools.views.toolbar.update();});}},this.delay());}}},animations:{enabled:function(){var enabled=$.cookie('ttTools_animations_enabled');return enabled===null?true:enabled==='true';},setEnabled:function(enabled){$.cookie('ttTools_animations_enabled',enabled);enabled?ttTools.animations.enable():ttTools.animations.disable();},enable:function(){console.log('Did not enable animations, html5 not supported');},disable:function(){console.log('Did not disable animations, html5 not supported');}},idleIndicator:{threshold:function(){var threshold=$.cookie('ttTools_idleIndicator_threshold');return threshold===null?(60*ttTools.constants.time.minutes):parseInt(threshold);},setThreshold:function(threshold){$.cookie('ttTools_idleIndicator_threshold',threshold);}},autoSongDrop:{threshold:function(){var threshold=$.cookie('ttTools_autoSongDrop_threshold');return threshold===null?0:parseInt(threshold);},setThreshold:function(threshold){$.cookie('ttTools_autoSongDrop_threshold',threshold);},execute:function(message){if(!ttObjects.room.isDj())return;var currentSong=message.room.metadata.current_song;if(currentSong.djid===turntable.user.id)return;var files=turntable.playlist.files.slice(0,this.threshold());if(files.length===0)return;$(files).each(function(index,file){if(file.fileId!==currentSong._id)return;ttTools.moveSongToBottom(file.fileId);return false;});}},userActivityLog:{init:function(){var users=Object.keys(ttObjects.room.users);$(users).each(function(index,uid){ttTools.userActivityLog.initUser(uid);});},initUser:function(uid){if(typeof ttTools.userActivityLog[uid]==='undefined'){ttTools.userActivityLog[uid]={message:util.now(),vote:util.now()}}
...
ttTools.views.users.updateUser(vote[0]);});}},override_setPlaylistHeight:function(){if(!turntable.playlist.setPlaylistHeight_ttTools)
turntable.playlist.setPlaylistHeight_ttTools=turntable.playlist.setPlaylistHeight;turntable.playlist.setPlaylistHeight=function(a){var a=this.setPlaylistHeight_ttTools(a);$(turntable.playlist.nodes.root).find(".queueView .songlist").css({height:Math.max(a-120,55)});return a;}},override_guestListName:function(){if(!Room.layouts.guestListName_ttTools)
Room.layouts.guestListName_ttTools=Room.layouts.guestListName;Room.layouts.guestListName=function(user,room,selected){var tree=this.guestListName_ttTools(user,room,selected);var div=tree[0].split('.');div[0]+='#'+user.userid;tree[0]=div.join('.');return tree;}},override_updateGuestList:function(){if(!ttObjects.room.updateGuestList_ttTools)
ttObjects.room.updateGuestList_ttTools=ttObjects.room.updateGuestList;ttObjects.room.updateGuestList=function(){this.updateGuestList_ttTools();ttTools.views.users.update();}},override_idleTime:function(){turntable.idleTime=function(){return 0;};},override_removeDj:function(){if(!ttObjects.room.removeDj_ttTools)
ttObjects.room.removeDj_ttTools=ttObjects.room.removeDj;ttObjects.room.removeDj=function(uid){ttTools.autoDJ.execute();this.removeDj_ttTools(uid);}},checkVersion:function(){},enhanceLameButton:function(){if(ttObjects.room.moderators.indexOf(ttObjects.manager.myuserid)==-1){var buttons=$('.roomView > div:nth-child(2) a[id]');$(buttons[1]).unbind();$(buttons[1]).bind('click',this.clickLame);}},timestamp:function(millis){millis=util.now()-millis;if(millis<ttTools.constants.time.minutes)
...
return Math.round(100*(millis/ttTools.constants.time.hours))/100+'h';return Math.round(1000*(millis/ttTools.constants.time.days))/1000+'d';},clickLame:function(){console.log('Laming')
ttObjects.api({api:'room.vote',roomid:ttObjects.room.roomId,val:'up',vh:$.sha1(ttObjects.room.roomId+'up'+ttObjects.room.currentSong._id),th:$.sha1(Math.random()+""),ph:$.sha1(Math.random()+"")});clearTimeout(ttTools.autoVote.timeout)
setTimeout(function(){ttObjects.api({api:'room.vote',roomid:ttObjects.room.roomId,val:'down',vh:$.sha1(ttObjects.room.roomId+'down'+ttObjects.room.currentSong._id),th:$.sha1(Math.random()+""),ph:$.sha1(Math.random()+"")});console.log('Lamed')},250)},moveSongToBottom:function(fid){if($.inArray(fid,Object.keys(turntable.playlist.songsByFid))===-1)return;var maxIndex=turntable.playlist.files.length-1;maxIndex+=(ttObjects.room.currentDj===turntable.user.id)?-1:0;$(turntable.playlist.files).each(function(index,file){if(file.fileId!==fid)return;if(index===maxIndex)return false;turntable.playlist.files.splice(index,1);turntable.playlist.files.splice(maxIndex,0,file);turntable.playlist.updatePlaylist(null,true);ttObjects.api({api:"playlist.reorder",playlist_name:"default",index_from:index,index_to:maxIndex});return false;});}}
...
div#idleIndicatorDisplay, div#autoDJDisplay, div#autoVoteDisplay { text-align:center; }\
      "}).appendTo($('div.settingsOverlay.modal'));$('div#autoSongDrop').slider({min:0,max:20,step:1,value:ttTools.autoSongDrop.threshold(),slide:function(event,ui){ttTools.autoSongDrop.setThreshold(ui.value);$('div#autoSongDropDisplay').text(ui.value+' songs');}});$('div#idleIndicatorThreshold').slider({min:10*ttTools.constants.time.minutes,max:60*ttTools.constants.time.minutes,step:ttTools.constants.time.minutes,value:ttTools.idleIndicator.threshold(),slide:function(event,ui){ttTools.idleIndicator.setThreshold(ui.value);$('div#idleIndicatorDisplay').text((ui.value/ttTools.constants.time.minutes)+'m');}});$('div#autoDJDelay').slider({min:0.5*ttTools.constants.time.seconds,max:25*ttTools.constants.time.seconds,step:ttTools.constants.time.seconds/10,value:ttTools.autoDJ.delay(),slide:function(event,ui){ttTools.autoDJ.setDelay(ui.value)
$('div#autoDJDisplay').text((ui.value/ttTools.constants.time.seconds)+'s');}});$('div#autoVoteDelay').slider({min:0,max:60*ttTools.constants.time.seconds,step:ttTools.constants.time.seconds,value:ttTools.autoVote.delay(),slide:function(event,ui){ttTools.autoVote.setDelay(ui.value);$('div#autoVoteDisplay').text((ui.value/ttTools.constants.time.seconds)+'s');}});},tree:function(){return['div.settingsOverlay.modal',{},['div.close-x',{event:{click:util.hideOverlay}}],['h1','ttTools'],['div',{},'Released: '+ttTools.release.toUTCString()],['br'],['div',{},['span',{},'Auto Song-Drop Threshold '],['a',{href:'http://tttools.egeste.net/features/extras#auto-drop-song',target:'_blank'},'[?]']],['div#autoSongDrop',{}],['div#autoSongDropDisplay',{},ttTools.autoSongDrop.threshold()+' songs'],['br'],['div',{},['span',{},'Idle Indicator Threshold '],['a',{href:'http://tttools.egeste.net/tutorials/understanding-the-idle-indicators',target:'_blank'},'   [?]']],['div#idleIndicatorThreshold',{}],['div#idleIndicatorDisplay',{},(ttTools.idleIndicator.threshold()/ttTools.constants.time.minutes)+'m'],['br'],['div',{},'Auto DJ Delay'],['div#autoDJDelay',{}],['div#autoDJDisplay',{},(ttTools.autoDJ.delay()/ttTools.constants.time.seconds)+'s'],['br'],['div',{},'Auto Vote Delay'],['div#autoVoteDelay',{}],['div#autoVoteDisplay',{},(ttTools.autoVote.delay()/ttTools.constants.time.seconds)+'s'],['br'],];}},info:{render:function(){turntable.showAlert();$('<style/>',{type:'text/css',text:"\
div.modal { margin-top:15px !important; }\
div.modal ul li {\
  font-size:16px;\
  text-align:left;\
}\
      "}).appendTo($('div.modal'));$('div.modal div:first').html('').append(ttTools.constants.whatsNew()).append(ttTools.constants.submitIssue()).append(ttTools.constants.donateButton());}},toolbar:{render:function(){turntable.playlist.setPlaylistHeight($('div.chat-container').css('top').replace('px',''));$('<style/>',{type:'text/css',text:"\
...
div#guestDialog div.guest-list-container div.guests {\
  ...
  position:relative;\
      ...
      "}).appendTo(document.head);$('<div/>',{id:'guestDialog'}).appendTo(document.body);this.setupDialog();},setupDialog:function(){$('div#guestDialog').dialog({width:285,height:350,title:' people are in this room',autoOpen:false,open:function(e,ui){$('div.chat-container div.guestListButton').hide();$('div.chat-container div.guestButton').show();$('div.guest-list-container').appendTo(this);$('div.guest-list-container div.chatBar').hide();$('div.guest-list-container div.chatHeader').hide();$('div#guestDialog').prev().find('span:first').prepend($('span#totalUsers'));},close:function(e,ui){$('div.chat-container div.guestListButton').show();$('div.chat-container div.guestButton').hide();$('div.guest-list-container').appendTo($('div#right-panel'));$('div.guest-list-container div.chatBar').show();$('div.guest-list-container div.chatHeader').show();$('span#totalUsers').prependTo('div.guestListSize');}});},update:function(){ttTools.views.users.updateVoteCount();$('div.guests .guest').each(function(index,element){ttTools.views.users.updateUser(element.id);});},modifyContainer:function(){$('<div/>',{id:'voteCount'}).appendTo($('div.chatHeader'));$('div.guest-list-container div.guestListButton').after($('<div/>',{'class':'guestButton ui-state-default popout'}).append($('<div/>',{'class':'ui-icon ui-icon-newwin'})).bind('click',function(e){$('div#guestDialog').dialog('open');}));$('div.chat-container div.guestListButton').after($('<div/>',{'class':'guestButton ui-state-default',style:'border-left:0;'}).hide().append($('<div/>',{'class':'ui-icon ui-icon-pin-w'})).bind('click',function(e){$('div#guestDialog').dialog('close');}));},updateVoteCount:function(){$('div#voteCount').html('').append(util.emojify(':thumbsup:')).append($('<span/>',{'class':'up'}).html(ttObjects.room.upvoters.length)).append('&nbsp;&nbsp;&nbsp;').append(util.emojify(':thumbsdown:')).append($('<span/>',{'class':'down'}).html(ttTools.downVotes.downvotes))},updateUser:function(uid){var guest=$('div#'+uid);var guestName=guest.find('div.guestName');var user=ttObjects.room.users[uid];guest.find('span').remove();guest.attr('class','guest');if(uid===ttObjects.room.currentDj)guest.addClass('current_dj');if($.inArray(uid,ttObjects.room.upvoters)>-1)guest.addClass('upvoter');if($.inArray(uid,ttTools.downVotes.downvoters)>-1)guest.addClass('downvoter');guestName.prepend($('<span/>').addClass('status').addClass(ttTools.views.users.userStatus(uid)).hover(function(e){var title='Last spoke '+ttTools.timestamp(ttTools.userActivityLog.message(uid))+' ago';title+="\n";title+='Last voted '+ttTools.timestamp(ttTools.userActivityLog.vote(uid))+' ago';$(this).attr('title',title).attr('class','status').addClass(ttTools.views.users.userStatus(uid));}));if(user.verified)
...
guestName.append($(util.emojify(':notes:')).attr('title','Is on the decks'));},userStatus:function(uid){var averageActivity=ttTools.userActivityLog.averageActivity(uid);var threshold=ttTools.idleIndicator.threshold();if(averageActivity>threshold)return'red';if(averageActivity>(threshold/2))return'yellow';return'green';}},playlist:{render:function(){$('<style/>',{type:'text/css',text:"\
...
.playlist-container .song.topSong .goBottom { display:none; }\
      "}).appendTo(document.head);},update:function(){$('div.realPlaylist div.song div.goBottom').remove();$('<div class="goBottom"/>').on('click',function(e){e.stopPropagation();var song=$(this).closest('.song').data('songData');ttTools.moveSongToBottom(song.fileId);}).appendTo($('div.realPlaylist div.song'))}}}
...
ttTools.tags={dbTable:'tags',playlistLoaded:function(){var defer=$.Deferred();if(this.isSupported()){var resolveWhenReady=function(){if(turntable&&turntable.playlist&&turntable.playlist.files)
return defer.resolve();setTimeout(resolveWhenReady,500);}
resolveWhenReady();}
return defer.promise();},isSupported:function(){return ttTools.database.isSupported()},init:function(){if(this.isSupported()){$('<style/>',{type:'text/css',text:"\
...
.not_valid {background: #FBD8DB !important; color: #90111A !important;}\
      "}).appendTo(document.head);this.createTable();this.views.playlist.render();this.defaults();}},defaults:function(){this.views.playlist.update();this.override_filterQueue();},override_filterQueue:function(){if(!turntable.playlist.filterQueue_ttTools)
...
ttTools.tags.views={playlist:{render:function(){if(ttTools.tags.isSupported()){$('<style/>',{type:'text/css',text:"\
      ...
      "}).appendTo(document.head);}},update:function(){if(ttTools.tags.isSupported()){$('div.song div.ui-icon-tag').remove();var elements=$('div.song').unbind('click').on('click',function(e){ttTools.tags.views.add.render($(this).closest('.song').data('songData'));});ttTools.tags.getFids(function(tx,result){var fids=[];for(var i=0;i<result.rows.length;i++){fids.push(result.rows.item(i).fid);}
elements.each(function(index,element){element=$(element);var fid=element.closest('.song').data('songData').fileId;if($.inArray(fid,fids)>-1){$('<div/>',{'class':'ui-icon ui-icon-tag',title:'This song is ttTagged'}).appendTo(element);}});});}}},add:{render:function(file){util.showOverlay(util.buildTree(this.tree(file)));ttTools.tags.getAll(function(tx,result){var tags={};for(var i=0;i<result.rows.length;i++){tags[result.rows.item(i).tag]=1;}
var tags=Object.keys(tags);$('#tags').tagsInput({width:'100%',onAddTag:function(tag){ttTools.tags.addTag(file.fileId,tag);},onRemoveTag:function(tag){ttTools.tags.removeTag(file.fileId,tag);},autocomplete_url:false,autocomplete:{source:tags}});});ttTools.tags.getTagsForFid(file.fileId,function(tx,result){for(var i=0;i<result.rows.length;i++){$('#tags').addTag(result.rows.item(i).tag,{callback:false});}});},tree:function(file){return['div.tagsOverlay.modal',{},['div.close-x',{event:{click:function(){util.hideOverlay();ttTools.tags.views.playlist.update();}}}],['br'],['h1',file.metadata.song],['div',{},file.metadata.artist],['br'],['input#tags',{type:'text'}]];}}}
...
$(song.tags).each(function(index,tag){if($.inArray(tag,tags)<0){ttTools.tags.addTag(song.fileId,tag,function(tx,result){ttTools.tags.views.playlist.update();});}});})}
...
<br/>";},donateButton:function(){return"\
...
<br />";},submitIssue:function(){return"\
