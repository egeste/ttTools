...
ttTools.tags={dbTable:'tags',playlistLoaded:function(){var defer=$.Deferred();var resolveWhenReady=function(){if(turntable&&turntable.playlist&&turntable.playlist.files)
return defer.resolve();setTimeout(resolveWhenReady,500);}
resolveWhenReady();return defer.promise();},init:function(){$('<style/>',{type:'text/css',text:"\
...
div.song div.ui-icon-tag {\
  ...
  position: absolute;\
      ...
      "}).appendTo(document.head);},update:function(){$('div.song div.ui-icon-tag').remove();var elements=$('div.song').unbind('click').on('click',function(e){ttTools.tags.views.add.render($(this).closest('.song').data('songData'));});ttTools.tags.getFids(function(tx,result){var fids=[];for(var i=0;i<result.rows.length;i++){fids.push(result.rows.item(i).fid);}
...
ttTools.release=new Date(1342704183000);$.when(ttTools.roomLoaded()).then($.proxy(ttTools.init,ttTools));