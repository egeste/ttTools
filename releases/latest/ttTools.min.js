...
return Math.round(100*(millis/ttTools.constants.time.hours))/100+'h';return Math.round(1000*(millis/ttTools.constants.time.days))/1000+'d';},moveSongToBottom:function(fid){if($.inArray(fid,Object.keys(turntable.playlist.songsByFid))===-1)return;$(turntable.playlist.files).each(function(index,file){if(index===turntable.playlist.files.length)return false;if(file.fileId!==fid)return;turntable.playlist.files.splice(index,1);turntable.playlist.files.push(file);turntable.playlist.updatePlaylist(null,true);ttObjects.api({api:"playlist.reorder",playlist_name:"default",index_from:index,index_to:turntable.playlist.files.length});return false;});}}
