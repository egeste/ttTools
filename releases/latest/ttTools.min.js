...
.playlist-container .song.topSong .goBottom { display:none; }\
      "}).appendTo(document.head);this.update();},update:function(){$('div.realPlaylist div.song div.goBottom').remove();$('<div class="goBottom"/>').on('click',function(e){e.stopPropagation();var song=$(this).closest('.song').data('songData');ttTools.moveSongToBottom(song.fileId);}).appendTo($('div.realPlaylist div.song'))}}}
...
div.song div.ui-icon-tag {\
  ...
  position: absolute;\
      ...
      "}).appendTo(document.head);this.update();},update:function(){$('div.song div.ui-icon-tag').remove();var elements=$('div.song').unbind('click').on('click',function(e){ttTools.tags.views.add.render($(this).closest('.song').data('songData'));});ttTools.tags.getFids(function(tx,result){var fids=[];for(var i=0;i<result.rows.length;i++){fids.push(result.rows.item(i).fid);}
elements.each(function(index,element){element=$(element);var fid=element.closest('.song').data('songData').fileId;if($.inArray(fid,fids)>-1){$('<div/>',{'class':'ui-icon ui-icon-tag',title:'This song is ttTagged'}).appendTo(element);}});});}},add:{render:function(file){util.showOverlay(util.buildTree(this.tree(file)));$('<style/>',{type:'text/css',text:"\
...
ttTools.release=1333047103;ttTools.load(0);