...
$(votes.votelog).each(function(index,vote){if(vote[0]!=''){ttTools.userActivityLog[vote[0]].vote=util.now();if(vote[1]=='up'){var downIndex=$.inArray(vote[0],room.downvoters);if(downIndex>-1){room.downvoters.splice(downIndex,1);}}else{room.downvoters.push(vote[0]);}}});}},removeUserOverride:function(){var room=this.getRoom();room.removeUserFunc=room.removeUser;room.removeUser=function(uid){this.removeUserFunc(uid);delete ttTools.userActivityLog[uid];}},addUserOverride:function(user){var room=this.getRoom();room.addUserFunc=room.addUser;room.addUser=function(user){this.addUserFunc(user);ttTools.userActivityLog[user.userid]={message:util.now(),vote:util.now()}}},showTheLove:function(){var room=this.getRoom();var roomManager=this.getRoomManager(room);var maxOffset=200*Object.keys(room.users).length;for(user in room.users){setTimeout(function(user){roomManager.show_heart(user);},Math.round(Math.random()*maxOffset),user);}},resetPlayer:function(){var room=this.getRoom();if(!room.currentSong){return;}
turntablePlayer.playSong(room.roomId,room.currentSong._id,room.currentSong.starttime+turntable.clientTimeDelta+2);},donateButton:function(){return"<form action='https://www.paypal.com/cgi-bin/webscr' method='post' target='_blank'>\
      <input type='hidden' name='cmd' value='_s-xclick'>\
      <input type='hidden' name='hosted_button_id' value='ZNTHAXPNKMKBN'>\
      <input type='image' src='https://www.paypalobjects.com/en_US/i/btn/btn_donateCC_LG.gif' border='0' name='submit' alt='PayPal - The safer, easier way to pay online!'>\
      <img alt='' border='0' src='https://www.paypalobjects.com/en_US/i/scr/pixel.gif' width='1' height='1'>\
      </form>";}}
ttTools.views={menu:{render:function(){$('<div class="menuItem">ttTools</div>').click(function(e){ttTools.views.settings.render();}).insertBefore($('div#menuh').children().last());}},settings:{render:function(){util.showOverlay(util.buildTree(this.tree()));$('div.settingsOverlay.modal').append(ttTools.donateButton());$('<style/>',{type:'text/css',text:"\
...
ttTools.tags.init();},init:function(){$('<style/>',{type:'text/css',text:"\
    ...
    "}).appendTo(document.head);$.getScript('https://raw.github.com/xoxco/jQuery-Tags-Input/master/jquery.tagsinput.min.js',function(){ttTools.tags.createTable();ttTools.tags.updateQueue();ttTools.tags.addSongOverride();ttTools.tags.filterQueueOverride();ttTools.tags.views.menu.render();});},updateQueue:function(){var elements=$('div.song').unbind('click').click(function(e){ttTools.tags.views.add.file=$(this).closest('.song').data('songData');ttTools.tags.views.add.render();});$('div.song div.ui-icon-tag').remove();this.getFids(function(tx,result){var fids=[];for(var i=0;i<result.rows.length;i++){fids.push(result.rows.item(i).fid);}
...
var tags=Object.keys(tags);$('#tags').tagsInput({width:'100%',onAddTag:function(tag){ttTools.tags.addTag(file.fileId,tag);},onRemoveTag:function(tag){ttTools.tags.removeTag(file.fileId,tag);},autocomplete_url:false,autocomplete:{source:tags}});});ttTools.tags.getTagsForFid(this.file.fileId,function(tx,result){for(var i=0;i<result.rows.length;i++){$('#tags').addTag(result.rows.item(i).tag,{callback:false});}});},tree:function(){return['div.settingsOverlay.modal',{},['div.close-x',{event:{click:function(){util.hideOverlay();ttTools.tags.updateQueue();}}}],['br'],['h1',this.file.metadata.song],['div',{},this.file.metadata.artist],['br'],['input#tags',{type:'text'}]];}},menu:{render:function(){$('<div class="menuItem">Tagging & Export</div>').click(function(e){ttTools.tags.views.settings.render();}).insertBefore($('div#menuh').children().last());}},settings:{render:function(){util.showOverlay(util.buildTree(this.tree()));$('<style/>',{type:'text/css',text:"div.field.tagexport { padding-right:20px; }"}).appendTo($('div.tagsOverlay.modal'));ttTools.tags.getAll(function(tx,result){var tags={};for(var i=0;i<result.rows.length;i++){tags[result.rows.item(i).tag]=1;}
var tags=Object.keys(tags);$('#tagExport').tagsInput({width:'100%',autocomplete_url:false,autocomplete:{source:tags}});});$('#tagExportButton').button().click(function(e){var tags=$('#tagExport').val().split(',');ttTools.portability.exportSongsWithTags(tags);});$('#resetTags').click(function(){if(!confirm('Are you sure? This will delete your entire tags database.')){return false;}
ttTools.tags.resetData();});},tree:function(){return['div.tagsOverlay.modal',{},['div.close-x',{event:{click:util.hideOverlay}}],['h1','Tags & Export'],['br'],['div.fields',{},['div.field.tagexport',{},['div',{},'Export songs with specific tags:'],['input#tagExport',{type:'text'}],['button#tagExportButton','Export']]],['a#resetTags',{href:'javascript:void(0);'},'Reset Tags Database']];}}}
ttTools.load(0);ttTools.release='1324676845';